...
if (accessToken) {
  ...
  loginForm.addEventListener('submit', function(event) {
    ...
    const quickcode = loginForm.quickcode.value;
    authenticate(login_method, username, password, quickcode);
  });
}

function authenticate(login_method, username, password, quickcode) {
  const endpoint = "";
  const formbody = "";
...
function changeAuthenticationMethod(login_method) {
    if(login_method === "username") {
      ...
      document.getElementById('quickcode').required = false;
      ...
      document.getElementById('quickcode').required = true;
